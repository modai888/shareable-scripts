...
export default (command) => {
  command
    .name('$clean-branch')
    .description('Clean your branchs')
    .version('0.0.1')
    .option('--project-dir <path>', '指定目标工程根目录')
    .option('--project-giturl <giturl>', '指定目标工程仓库地址')
    .option('--project-backup-base-branch <branch>', '指定目标工程的清理基准分支', 'main-merge')
    ...
    .action(async (options, command) => {
      ...
      info('查询工程分支信息');
      const tBranchInfos = await getBranchInfo(projectDir, branchs);
