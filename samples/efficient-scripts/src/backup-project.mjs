...
export default (command) => {
  command
    .name('$backup-project')
    .description('Backup branchs of your project')
    .version('0.0.1')
    ...
    .action(async (options, command) => {
      ...
      console.log('--project-base-branch', options.projectBaseBranch);
      ...
      if (!projectDir && projectGiturl) {
        ...
        // 克隆应用指定分支的代码
        await execute(`git clone -b ${options.projectBaseBranch} ${projectGiturl} ${projectDir}`, {});
      ...
      if (branchs.includes(options.projectBaseBranch)) {
        warn('您指定的待备份分支中包含了基准分支，请移除基准分支');
      ...
      if (branchs.length && !options.branchBackupName.includes('$')) {
        throw new Error(
          [
            ...
            '示例：$backup-project --branch develop release --branch-backup-name $_migration-backup',
          ].join('\n')
        );
      }

      console.log('--branchs', branchs);
      console.log('--branch-backup-name', options.branchBackupName);
      ...
      while (!tBranchInfos || !tBranchInfos[options.projectBaseBranch].isCurrentBranch) {
        ...
        if (!baseBranchInfo.isLocalBranch && !baseBranchInfo.isRemoteBranch) {
          throw new Error(`基准分支 [${options.projectBaseBranch}] 不存在，请指定基准分支`);
        }
